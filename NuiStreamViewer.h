...
public:
	...
	bool GetSkeletonTrackingStatus() {return m_state == NUI_SKELETON_TRACKED;}
	const NUI_SKELETON_FRAME* getSkeleton() const {return m_pSkeletonFrame;}
	...
	void GetFallDetect(InbedAPPs** ppFallDetect) const {*ppFallDetect = m_pInbedAPPs;};
	void GetLegRaisExcer(LegRaisExcer** ppLegRaisExcer) const {*ppLegRaisExcer = m_pLegRaisExcer;}
	void GetHandRaisExcer(HandRaisExcer** ppHandRaisExcer) const {*ppHandRaisExcer = m_pHandRaisExcer;}
...
private:
    ...
    /// <param name="skeletonData">Skeleton coordinates</param>
    /// <param name="imageRect">The rect which the color or depth stream image is streched to fit</param>
    void DrawHandsPosition(const NUI_SKELETON_DATA& skeletonData, const D2D1_RECT_F& imageRect);
	...
	/////Save skeleton data joint by joint
	void SaveJointPos(const NUI_SKELETON_DATA& skeletonData, const NUI_SKELETON_FRAME& skeletonFrame);

	////Draw Leg Raising Practice messages on screen
	void DrawLegRaisMeg(const RECT& clientRect);

	////Draw Hand Raising Practice messages on screen
	void DrawHandRaisMeg(const RECT& clientRect);

	////Draw Fall detection messages on screen
	void DrawDetectionMeg(const RECT& clientRect);

	////Draw Hands Movement RIC messages on screen
	void DrawHandsMovementRICMeg(const D2D1_RECT_F& imageRect, const RECT& clientRect);

	////Txt to speech
	//BOOL Txt2SpeechThread(LPCWSTR pFileName);
...
private:
	...
	// Leg raising excercise
	LegRaisExcer*               m_pLegRaisExcer;

	// Hand raising excercise
	HandRaisExcer*              m_pHandRaisExcer;

	// Fall detection
	InbedAPPs*                  m_pInbedAPPs;

    ImageRenderer*              m_pImageRenderer;

	NUI_SKELETON_TRACKING_STATE m_state;
	enum ImageRendererBrush     m_ImageRendererBrush;
